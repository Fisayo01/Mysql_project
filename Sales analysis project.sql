...
left join sales_team 
  on orders.salesteam_id = sales_team.salesteam_id
left join product 
  on orders.product_id = product.product_id
left join store 
  on orders.store_id = store.store_id;
...
where order_quantity > (
  select round(avg(Order_quantity)
  ) 
from orders);
...
select 
  ...
  sum(order_quantity) as total_qty_ordered
from sales_info
group by product_name
...
-- 12. Extract the product_name, sales_price and calculate the profit Percentage
select 
	Product_name, 
	sales_price, 
  profit, 
	((profit/sales_price) * 100) as Profit_Percentage
from sales_details;

-- 13. Extract Product name and the profitable and non-profitable sales
Select 
	product_name, 
	case
		when (sales_price - cost_price) > 100 then 'Profitable'
		else 'Non-Profitable'
